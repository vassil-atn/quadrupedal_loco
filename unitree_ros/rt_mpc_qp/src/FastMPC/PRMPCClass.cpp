...
{    
    ...
    double stepwidth = 2*_RobotParaClass_HALF_HIP_WIDTH;
    double steplengthx = 0;
    double stepheight = 0.0;
    double _lift_height = 0.015; //// for bipedal walking and troting
    //double _lift_height = 0.01; //// for bounding
...
Eigen::Matrix<double, 30, 1> PRMPCClass::Foot_trajectory_solve_mod2(int j_indexx,bool _stopwalking, Eigen::Matrix<double, 9, 1> Nrtfoorpr_gen)
{
  ...
  _t_end_footstep = round((_tx(_footstepsnumber-1)- 2*_tstep)/_dt_mpc);  
...
Eigen::Matrix<double, 30, 1> PRMPCClass::XGetSolution_Foot_rotation(int walktimex, double dt_sample)
{
  ...
  for (int walktime = walktimex; walktime < walktimex+jx_max; walktime ++)
  {
	...
	_Rfoot_r(0, walktime - walktimex) =  -0.065*(1 - cos(2*M_PI / (_ts(_bjx1-1,0) ) * (t_desxx + 2*_td(_bjx1-1,0)/4) ));
  ...
  //       _Rfoot_ra(0, walktime - walktimex) =  -0.065*(2*M_PI / (_ts(_bjx1-1,0) ) * 2*M_PI / (_ts(_bjx1-1,0) ) * cos(2*M_PI / (_ts(_bjx1-1,0) ) * (t_desxx + 2*_td(_bjx1-1,0)/4) ));
	...
	if ((t_desxx + 2*_td(_bjx1-1,0)/4) >= (_ts(_bjx1-1,0)/2))
	{
	  if ((_footxyz_real(0,_bjx1)-_footxyz_real(0,_bjx1-1))>0)
	  {
	    ...
	    _Rfoot_r(1, walktime - walktimex) =  0.075*(_footxyz_real(0,_bjx1)-_footxyz_real(0,_bjx1-1))/(_footx_max) * (cos(4*M_PI / (_ts(_bjx1-1)) * (t_desxx + 2*_td(_bjx1-1)/4) ) - 1);  
  ...
  /*	_Rfoot_rv(1, walktime - walktimex) = 0;
	...
	//// left foot roll:
	/////// step forward and backward 1//////
	_Lfoot_r(0, walktime - walktimex) =  0.075*(1 - cos(2*M_PI / (_ts(_bjx1-1,0) ) * (t_desxx + 2*_td(_bjx1-1,0)/4) ));
  /*      _Lfoot_rv(0, walktime - walktimex) =  -0.075*(2*M_PI / (_ts(_bjx1-1,0) ) * sin(2*M_PI / (_ts(_bjx1-1,0) ) * (t_desxx + 2*_td(_bjx1-1,0)/4) ));
	...
	if ((t_desxx + 2*_td(_bjx1-1,0)/4) >= (_ts(_bjx1-1,0)/2)) ///later half cycle: minus angle;
	{
	  if ((_footxyz_real(0,_bjx1)-_footxyz_real(0,_bjx1-1))>0)
	  {
	    ...
	    _Lfoot_r(1, walktime - walktimex) =  0.075*(_footxyz_real(0,_bjx1)-_footxyz_real(0,_bjx1-1))/(_footx_max) * (cos(4*M_PI / (_ts(_bjx1-1,0)) * (t_desxx + 2*_td(_bjx1-1,0)/4)) - 1); 
  ...
  /*	_Lfoot_rv(1, walktime - walktimex) = 0;
    ...
    footlr_r_inte(2+ 6*(walktime - walktimex),0) = _Rfoot_r(2,0);
    ...
    footlr_r_inte(5+ 6*(walktime - walktimex),0) = _Lfoot_r(2,0);    
