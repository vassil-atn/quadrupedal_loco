        ...
        if (control_gait(0)>0)
        {
            ...
            if ( mpc_gait_flag>0) /// receieved data from the low MPC;
            {
                ...
                if ( count_in_rt_mpc * dt_mpc_fast > _height_offset_timex)
                {
                  ...
                  //foorpr_gen = rpympc.Foot_trajectory_solve_mod1(count_in_rt_mpc_foot, _stopwalking,Nrtfoorpr_gen);
...
//         low_mpc_gait_inte(33,0) = rftheta_inter(2);
        ...
        joint2simulationx.position[98] = (int) rpympc._tx_total/gait::t_program_cyclic;
